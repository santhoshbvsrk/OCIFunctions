...
def call_oic(oicbaseurl, oicusername, oicuserpwd, empid, deptno):
    response = {'status':None, 'description':None, 'output':None}
    
    try:
        auth = (oicusername,oicuserpwd)
        resp= requests.get(oicbaseurl,params={"EmpId":empid,"DeptNo":deptno},headers={'type': 'application/json'},auth=auth)

        logging.getLogger().info('INFO: Calling FetchEmpDeptData Integration is Successful: {0}'.format(str(resp.status_code)))
